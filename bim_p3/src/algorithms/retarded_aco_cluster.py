...
def run_aco(
    ...
    rho: float = 0.15,
    q: float = 100.0,
    pheromone: list[list[float]] | None = None,
    seed: int = 0,
) -> tuple[list[list[int]], list[float], list[list[float]], list[list[float]]]:
    """
    ...
    return best_routes, history, pheromone, heuristic
...
def run_cluster(
    ...
    """
    Recursive ACO algorithm that clusters customers into multiple subproblems (), then runs ACO on each cluster to find good routes.
    """
    n = problem.num_customers
    ...
    # Now, the algorithm should combine the pheromone information from the subproblems to construct a global solution.
    # We execute a final ACO run on the original problem, using the pheromone information from the subproblems to bias the search.
    # For simplicity, we will just take the best routes from each subproblem and combine them, then refine the combined routes.

    pheromone = np.full((n + 1, n + 1), 0.10)
    # pheromone = [[0.15 for _ in range(n + 1)] for _ in range(n + 1)]
    routes = [route[0][0] for route in subproblem_results]
    for subproblem, subproblem_results, route in zip(subproblems, subproblem_results, routes):
